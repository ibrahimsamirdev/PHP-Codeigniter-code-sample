...
$(document).ready(function() {
	...
	function submit_form(querystr){
		...
		$.ajax({
			url		:	"<?=site_url('countries/submit/'.@$id)?>",
			type	:	"POST",
			dataType:	"json",
			data	:	querystr,
			...
			$("#loading div").fadeOut(300);
				if(data.server_validation)
				{
					...
					if(!data.is_valid)
					{
						$.validationEngine.buildPrompt('#name','Country already exists','error');
					}
					else{
						load_content('countries');
					}
				}
			}
		});
...
<div id="container">
	...
	<div class="form_title"><?=@$id?"Edit Country":"Create Country"?></div>
	<div class="form">
	<form action="<?=site_url('countries/submit/'.@$id)?>" method="post" onsubmit="return false" id="form">
		<br />
		<div class="sepa">
		...
		&nbsp;&nbsp;
		<input name="cancel" type="button" id="cancel" value="Cancel" onclick="load_content('countries')" />
		</div>
	</form>
	</div>
	</div>
