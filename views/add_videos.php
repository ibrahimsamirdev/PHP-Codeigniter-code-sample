...
function upload_response(){
	...
	//this is to do nothing on the first run
	...
	if(!data.is_valid)
	{
		...
		if(data.error2)
			$.validationEngine.buildPrompt('#userfile2',data.error2,'error');
	}else{
		////////////// DEBUG //////////////
		...
		load_content('media/get_media/videos/<?=$entity_type?>/<?=$entity_id?>','#tab_loader','#form_loader');
		hs.close();
	}
}
</script>
  ...
  <div class="form_block2">
	<div class="form">
	<form action="<?=site_url('media/upload_media/videos/'.@$entity_type.'/'.@$entity_id.'/'.@$id)?>" method="post" enctype="multipart/form-data" target="upload_target"  id="form2" >
	  <br />
	  <div class="sepa">
	  ...
	  <div class="sepa">
		...
		<div class="error">(*.flv, *.mp4, *.mov) [16MB]</div>
	  </div>
		
	  <div class="sepa">
        <label for="name">Cover Image </label>
        <input name="userfile2" type="file" id="userfile2" class="file" />
	    <div class="error">(*.jpg, *.png, *.gif) [1MB]</div>
      </div>
	  <div class="sepa">
